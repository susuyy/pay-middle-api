...
import com.jlpay.ext.qrcode.trans.request.PreauthCloseRequest;
import com.jlpay.ext.qrcode.trans.response.PreauthCloseResponse;
...
public class PreauthCloseService {
    ...
    public static void main(String[] args) {
        //组装请求参数
        PreauthCloseRequest request = componentRequestData();
        //交易请求
        PreauthCloseResponse response = TransExecuteService.executor(request, PreauthCloseResponse.class);
        System.out.println("返回参数=========>" + JSON.toJSON(response));

    }

    private static PreauthCloseRequest componentRequestData() {
        PreauthCloseRequest request = new PreauthCloseRequest();
        //必传字段
        ...
        request.setOriOutTradeNo("ZS5860278521");//商家系统内部原订单号   机构下唯一
        request.setOriTransactionId("8001158220200914103858264133");//原预授权嘉联订单号，
        request.setGuaranteeAuthCode("007462");//原授权码交易返回的授权码
